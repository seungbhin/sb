...
def load_cogvideox_pipeline_with_lora(args, dtype=torch.bfloat16):
    """
    Base CogVideoX + LoRA adapter를 로드하는 함수.
    """
    # 1. 기본 CogVideoX 컴포넌트 로드
    cogvideox_tokenizer = AutoTokenizer.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="tokenizer", revision=args.revision
    )
    ...
    # 여기서는 Custom TransformerWithAdapter 대신,
    # base transformer를 pipeline 안에서 생성/관리한다고 가정 (model.pipeline.CogVideoXPipeline 내부).
    pipe = CogVideoXPipeline(
        tokenizer=cogvideox_tokenizer,
        text_encoder=cogvideox_text_encoder,
        vae=cogvideox_vae,
    ...
    # 2. LoRA weight 로드
    if args.lora_path:
        adapter_name = args.lora_adapter_name or "cogvideox-lora"
        print(f"\033[1;31m Loading LoRA from: {args.lora_path} (adapter_name={adapter_name}) \033[0m")
        pipe.load_lora_weights(args.lora_path, adapter_name=adapter_name)
        pipe.set_adapters([adapter_name], [args.lora_scale])
        print(f"\033[1;31m Set LoRA adapter scale: {args.lora_scale} \033[0m")
    else:
        print("\033[1;33m No LoRA path provided. Running base CogVideoX only. \033[0m")
